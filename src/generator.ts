...
function buildMemoryRefs(answers: OnboardingAnswers): MemoryRefs {
  const refs: MemoryRefs["refs"] = [
    {
      type: "semantic",
      uri: "./memory/identity.json",
...
function buildMemoryIdentityJson(slug: string, answers: OnboardingAnswers, profile: Profile): string {
  const entries: MemoryEntry[] = [
    {
      id: `${slug}-handle`,
      summary: `Handle is ${slug} on Moltbook. Profile URL: https://www.moltbook.com/u/${slug}`,
      memory_type: "identity",
      tags: ["identity", "handle", "moltbook"],
      priority: 0.95,
      source_title: "Generated by moltbook-onboarding"
    },
    {
      id: `${slug}-role`,
      summary: `Role: ${answers.role}. Goal: ${answers.moltbookGoal}`,
      memory_type: "identity",
      tags: ["identity", "role", "goal"],
      priority: 0.9,
      source_title: "Generated by moltbook-onboarding"
    },
    {
      id: `${slug}-voice`,
      summary: `Tone: ${profile.speech_style.tone}. Style: ${profile.speech_style.style.join(", ")}. Signature line: ${answers.starterLine}`,
      memory_type: "identity",
      tags: ["identity", "voice", "style", "tone"],
      priority: 0.88,
      source_title: "Generated by moltbook-onboarding"
    },
    {
      id: `${slug}-audience`,
      summary: `Audience: ${answers.audience}. Concept: ${answers.concept}`,
      memory_type: "identity",
      tags: ["identity", "audience", "concept"],
      priority: 0.85,
      source_title: "Generated by moltbook-onboarding"
    },
    {
      id: `${slug}-topics`,
      summary: `Topics to naturally gravitate toward: ${answers.topics.join(", ")}`,
      memory_type: "identity",
      tags: ["identity", "topics", ...answers.topics.map((t) => t.toLowerCase().replace(/\s+/g, "_"))],
      priority: 0.82,
      source_title: "Generated by moltbook-onboarding"
    },
    {
      id: `${slug}-boundaries`,
      summary: `Hard boundaries — never cross these: ${answers.boundaries.join(", ")}`,
      memory_type: "identity",
      tags: ["identity", "boundaries", "safety"],
      priority: 0.95,
      source_title: "Generated by moltbook-onboarding"
    },
    {
      id: `${slug}-posting`,
      summary: `Posting style: ${answers.postingStyle}`,
      memory_type: "identity",
      tags: ["identity", "posting", "moltbook"],
      priority: 0.8,
      source_title: "Generated by moltbook-onboarding"
    }
  ];
...
function buildMemoryInteractionsJson(): string {
  return JSON.stringify([], null, 2);
}

function buildMemoryRelationshipsJson(): string {
  return JSON.stringify([], null, 2);
}
    ...
    rules: [
      {
        ...
        priority: 100,
        when: { emotion: "confidence", gte: 0.75 },
        effect: {
      ...
      },
      {
        ...
        priority: 80,
        when: { context_key: "conflict_style", eq: answers.conflictStyle },
        effect: {
          behavior_biases: {
...
function buildLaunchPrompt(slug: string, answers: OnboardingAnswers): string {
  ...
  return [
    ...
    "2. Load and apply the APP session bundle from session.json. This defines your personality, state, and behavior rules.",
    "3. Load your APP memory using memoryrefs.json. Each ref has a uri pointing to a local JSON file:",
    "   - memory/identity.json (semantic) — stable facts about who you are. Read at session start. Never overwrite.",
    "   - memory/interactions.json (episodic) — log of past activity. Read at start, append new entries at end.",
    "   - memory/relationships.json (social) — people worth knowing. Read at start, update entries as relationships develop.",
    "   Also load memory.md for a human-readable summary of the same.",
    ...
    "9. After the session: append a new entry to memory/interactions.json for each post or comment made. Update memory/relationships.json with any new people worth knowing. Update memory.md to match.",
    "",
    ...
    "- ./session.json         ← APP bundle: profile, state, policy all in one",
    "- ./memoryrefs.json      ← tells you where your memory files are",
    "- ./memory/identity.json ← stable identity facts, read-only",
    "- ./memory/interactions.json ← past activity log, append each session",
    "- ./memory/relationships.json ← people worth knowing, update as needed",
    "- ./memory.md            ← human-readable memory summary, keep in sync",
...
function buildOpenclawAgentsMd(slug: string, answers: OnboardingAnswers): string {
  return [
    ...
    "",
    "## APP memory files",
    "These files live alongside this kit and hold your structured APP memory.",
    "Load them at session start. Write back to them before finishing.",
    "",
    "- memory/identity.json   — stable identity facts (read-only, never overwrite)",
    "- memory/interactions.json — past posts and comments (append new entries each session)",
    "- memory/relationships.json — people worth knowing (update as relationships develop)",
    "",
    "Each entry in those files follows the APP memory entry format:",
    "  { id, summary, memory_type, tags, priority, source_title }",
    "",
    "## People worth knowing",
    "<!-- Mirrors relationships.json in human-readable form. Keep in sync. -->",
    "",
    "## Recent activity",
    "<!-- Mirrors interactions.json in human-readable form. Keep in sync. -->",
    "",
    "## Notes",
    "<!-- Anything else worth carrying forward between sessions -->"
  ].join("\n");
...
export function generateKit(answers: OnboardingAnswers): GeneratedKit {
  ...
  const memoryRefs = buildMemoryRefs(answers);
  ...
  return {
    ...
    memoryIdentityJson: buildMemoryIdentityJson(slug, answers, profile),
    memoryInteractionsJson: buildMemoryInteractionsJson(),
    memoryRelationshipsJson: buildMemoryRelationshipsJson()
  };
}
...
export async function writeKit(baseDir: string, kit: GeneratedKit): Promise<string> {
  ...
  await Promise.all([
    ...
    writeFile(path.join(memoryDir, "identity.json"), kit.memoryIdentityJson + "\n"),
    writeFile(path.join(memoryDir, "interactions.json"), kit.memoryInteractionsJson + "\n"),
    writeFile(path.join(memoryDir, "relationships.json"), kit.memoryRelationshipsJson + "\n")
  ]);
